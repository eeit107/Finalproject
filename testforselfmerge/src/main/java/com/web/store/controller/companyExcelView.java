...
public class companyExcelView extends AbstractXlsView  {
	...
	private void populateExcelCells(Map<String, Object> model, Workbook workbook) {
        ...
        List<ShopmemberBean> Shopmember = (List<ShopmemberBean>) model.get("company");
        
		Set<String> set = model.keySet();
		Row row = null;
		Cell cell = null;
		
		for(ShopmemberBean m : Shopmember) {
			colCount = 0;
			//for(ShopmemberBean m : Shopmember) Shopmember存放多個bean,new一個ShopmemberBean接多個Shopmember,ShopmemberBean m(變數名)
			// m.get出要顯示在excel上的欄位
			...
			cell = row.createCell(colCount++);
			cell.setCellStyle(styleCenter);
			cell.setCellValue(m.getCompanyname());			
			
			cell = row.createCell(colCount++);
			cell.setCellStyle(styleCenter);
			cell.setCellValue(m.getCompanytel());			
...
//			cell.setCellValue(m.getBirthday());
	...
	private void createExcelHeaders(Workbook workbook) {
		...
		String[] labels = {"帳號", "姓名","公司名稱","公司電話","信箱","公司地址","統一編號"};
